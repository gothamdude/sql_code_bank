CREATE OR REPLACE FUNCTION CALC_QUARTER_HOUR(HOURS IN NUMBER) RETURN NUMBER AS CALCULATED_HOURS NUMBER := 0;
BEGIN
-- if HOURS is greater than one, then calculate the decimal portion based upon quarterly hours
IF HOURS > 1 THEN
-- calculate the modulus of the HOURS variable and compare it to  fractional values
    ...
    END IF;
ELSE
-- if HOURS is less than one, then calculate the entire value based upon quarterly hours
